...
class App extends React.Component {
  ...
  render() {
    ...
    let filteredPokemonListMenuChoice =
      filterValue === "all"
        ? this.state.pokemon
        : this.state.pokemon.filter((pokemon) => {
            return pokemon.type === filterValue;
          });

    const searchText = this.state.textSearch;
    let filteredPokemonListTextChoice = filteredPokemonListMenuChoice.filter(
      (pokemon) => pokemon.name.startsWith(searchText)
    );
    ...
    let pokemonList = filteredPokemonListTextChoice
      ...
      .map((pokemon, index) => {
        return (
          <div key={index} className="pokemon-items">
    ...
    return (
      <div className="content-container">
        ...
        <div className="userChoice-section">
          <div className="sort-section">
            <label style={{ marginLeft: "10px" }}>
              Sort by:
              <select
                onChange={this.handleSortValue.bind(this)}
                style={sortStyle}
              >
          ...
          <div className="filter-section">
            <label>
              Filter by type:
              <select
                onChange={this.handleFilterValue.bind(this)}
                style={sortStyle}
              >
          ...
          <div className="text-section">
            ...
            <button
              onClick={this.handleClickRestart.bind(this)}
        ...
        <div className="pokemon clearfix">{pokemonList}</div>
      </div>
    );
  }
}
